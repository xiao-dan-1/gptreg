#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""DOM 结构差分：browser 有 10 scripts + 10 body children，vm 只有 1 script。

验证 document.scripts / body 结构是否是 vm t 短 224B 的来源。
用法: python capture/t_dom_exp.py
"""
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        dom_info = page.evaluate("""() => ({
            ...
            n_scripts: document.scripts.length,
            n_body: document.body ? document.body.children.length : 0,
            title: document.title,
            meta: [...document.querySelectorAll('meta')].map(m => (m.name||m.httpEquiv||'')).filter(Boolean),
        })""")
    ...
    scripts = dom_info.get("scripts") or []
    print(f"browser scripts={dom_info.get('n_scripts')} body_children={dom_info.get('n_body')} "
          f"title={dom_info.get('title')!r} meta={dom_info.get('meta')}", flush=True)
    print(f"scripts src: {[s[:60] for s in scripts]}", flush=True)

    # 构造 vm 组
    base_scripts = scripts[1:] if len(scripts) > 1 else []  # 去掉 vm 已有的 loader(scripts[0])
    groups = {
        "G0_base": {},
        "G1_scripts": {"extra_scripts": base_scripts},
        "G2_scripts_full": {"extra_scripts": scripts},
    }
    ...
    (ROOT / "data" / "t_dom_exp_result.json").write_text(json.dumps({
        "device_id": device_id, "browser_t_b64": browser_t,
        "dom_info": dom_info, "groups": results,
    }, ensure_ascii=False, indent=1), encoding="utf-8")
    print("\n结果已存 data/t_dom_exp_result.json", flush=True)
    return 0
