#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""种子字段级差分：定位 rctx/localStorage/font 各子字段对 t 的贡献。

回答：
  1. rctx 贡献的 ~49B 来自「键存在」(window 键采样) 还是「内容」(blob2 读取)？
  2. localStorage 各键 / font 值各贡献多少？

用法: python capture/t_field_exp.py
"""
...
def main() -> int:
    ...
    ld = (rctx.get("state") or {}).get("loaderData")
    groups = {
        "G0_base": {},
        "G1_rctx_empty": {"react_router_full": {}},
        "G2_rctx_state": {"react_router_full": {"state": {"loaderData": {}, "actionData": None, "errors": None}}},
        "G3_rctx_ld": {"react_router_full": {"state": {"loaderData": ld, "actionData": None, "errors": None}}},
        "G4_rctx_full": {"react_router_full": rctx},
        "G5_+ls": {"react_router_full": rctx, "ls_extra": seed.get("ls_extra") or {}},
        "G6_+font": {"react_router_full": rctx, "ls_extra": seed.get("ls_extra") or {}, "font_gbcr": seed.get("font_gbcr") or {}},
    }
    ...
    (ROOT / "data" / "t_field_exp_result.json").write_text(json.dumps({
        "device_id": device_id, "browser_t_b64": browser_t, "groups": results,
    }, ensure_ascii=False, indent=1), encoding="utf-8")
    print("\n结果已存 data/t_field_exp_result.json", flush=True)
    return 0
